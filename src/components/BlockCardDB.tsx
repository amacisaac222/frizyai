...
import { BlockStatus } from '@/lib/database.types'
import { cn } from '@/utils'

interface BlockCardDBProps {
  block: Block
...
export function BlockCardDB({ 
  ...
  const hasClaudeSessions = block.claude_sessions > 0
  const isRecentlyWorked = block.last_worked && 
    (new Date().getTime() - new Date(block.last_worked).getTime()) < 24 * 60 * 60 * 1000
  const isBlocked = block.status === BlockStatus.blocked || block.blocked_by.length > 0
  const hasRecentSessions = block.claude_sessions > 0 && isRecentlyWorked
      ...
      <CardContent className="p-4">
        ...
        <div className="flex items-center justify-between text-xs text-muted-foreground pt-2 border-t border-border/50">
          <div className="flex items-center gap-3">
            ...
            {hasClaudeSessions && (
              <div className="flex items-center gap-1 text-orange-600" title={`${block.claude_sessions} Claude sessions`}>
                <Zap className="w-3 h-3" />
                <span>{block.claude_sessions}</span>
              </div>
            )}
            
            {/* Last worked timestamp */}
            {block.last_worked && (
              <div className="flex items-center gap-1" title={`Last worked: ${new Date(block.last_worked).toLocaleString()}`}>
                {isRecentlyWorked && <div className="w-2 h-2 rounded-full bg-green-500" />}
                <span>
                  {isRecentlyWorked 
                    ? 'Recent' 
                    : new Date(block.last_worked).toLocaleDateString()
                  }
                </span>
              </div>
            )}
        ...
        {block.ai_suggestions && Array.isArray(block.ai_suggestions) && block.ai_suggestions.length > 0 && (
          <div className="mt-2 pt-2 border-t border-border/50">
            <div className="flex items-center gap-1 text-xs text-blue-600">
              <MessageSquare className="w-3 h-3" />
              <span>{block.ai_suggestions.length} AI suggestion{block.ai_suggestions.length !== 1 ? 's' : ''}</span>
            </div>
          </div>
        )}
      </CardContent>
